# v0.5: SLDPRT Geometry Parser & Viewer (read-only prototype)
...
- Not a converter. There is no SLDPRT writer and no export path back into the
  SolidWorks format.
- Not a complete parser. It only extracts what `KNOWN_INVARIANTS.md` has
...
- **Legacy OLE2 container** (SW2000-s01.SLDPRT, plate4.sldprt,
  chainwheel.sldprt in the current corpus). `RESEARCH_DASHBOARD.md`'s
...
2. `test/compare-with-reference.js` -- per-face parity: every one of the
   ...
   exist in the v0.5 parser's output at the identical marker offset, with
