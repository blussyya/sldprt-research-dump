 ...
 * the v0.4.5 reference, it confirms the v0.5 parser extracts a face at the
...
// file (relative path, as used by v0.4.5 scripts) -> shortName used by v0.5's file list
const FILES = [
...
for (const f of FILES) {
  ...
  console.log('\n--- ' + f.shortName + ' --- reference: ' + refFaces.length + ' faces, v0.5: ' + v5Faces.length + ' faces');

  // Every reference face must exist in v0.5 output at the same mp, with the
  // same secCount.
  for (const [mp, rf] of refByMp) {
    ...
    if (!vf) {
      mismatches++;
      console.error('  MISMATCH: reference face at mp=0x' + mp.toString(16) + ' missing from v0.5 output');
      continue;
    }
    if (vf.secCount !== rf.secCount) {
      mismatches++;
      console.error('  MISMATCH: mp=0x' + mp.toString(16) + ' secCount reference=' + rf.secCount + ' v0.5=' + vf.secCount);
    }
    if (vf.edgeCount !== rf.ec) {
      mismatches++;
      console.error('  MISMATCH: mp=0x' + mp.toString(16) + ' edgeCount reference=' + rf.ec + ' v0.5=' + vf.edgeCount);
    }
    if (vf.vertexCount !== rf.vc) {
      mismatches++;
      console.error('  MISMATCH: mp=0x' + mp.toString(16) + ' vertexCount reference=' + rf.vc + ' v0.5=' + vf.vertexCount);
    }
  }

  // Every v0.5 face must exist in the reference (no extras).
  for (const [mp] of v5ByMp) {
    if (!refByMp.has(mp)) {
      mismatches++;
      console.error('  MISMATCH: v0.5 face at mp=0x' + mp.toString(16) + ' not present in reference');
    }
  }
}

console.log('\n' + '='.repeat(70));
console.log('Compared ' + totalCompared + ' reference faces against v0.5 parser output.');
if (mismatches === 0) {
