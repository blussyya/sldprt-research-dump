...
function _processPlaneFace(face, vertices, surfaceInfo) {
    ...
    // ================================================================
    // PATH B: No STEP data — Topology Edge-Loop Tracer via strip
    // ================================================================

    // Step 1: Generate strip triangles (same alternating-winding logic as _processStripFace)
    const stripTris = [];
    for (let i = 0; i < n - 2; i++) {
        const i0 = face[i], i1 = face[i + 1], i2 = face[i + 2];
        if (i0 === i1 || i1 === i2 || i0 === i2) continue;
    ...
    // Step 2: Build edge usage frequency map (direction-agnostic keys)
    ...
    // Step 3: Collect boundary edges (used exactly 1×)
    const boundaryEdges = [];
    ...
    // Step 4: Chain boundary edges into closed loops (endV == startV)
    const adj = new Map();
    ...
    // Step 5: Classify loops by 2D signed area
