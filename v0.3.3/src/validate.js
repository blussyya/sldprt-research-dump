...
function parseArgs(args) {
    const opts = { files: [], tolerance: 0.5, format: 'binary-stl', verbose: false, output: null, outputDir: null };
    let i = 0;
    while (i < args.length) {
        const a = args[i];
        if (a === '-v' || a === '--verbose') opts.verbose = true;
        else if (a === '-o' || a === '--output') opts.output = args[++i];
        else if (a === '-d' || a === '--output-dir') opts.outputDir = args[++i];
...
function run() {
    ...
    if (args.length === 0) {
        console.log(`SLDPRT Validate & Convert (v0.3.2)
...
Options:
  ...
  -d, --output-dir <dir>  Output directory (default: alongside input)
...
Examples:
  ...
  node validate.js part.sldprt -d ./output        # export to ./output
  node validate.js part.sldprt --format obj       # export as OBJ`);
    ...
    if (stepPath) {
        ...
        for (const slF of slFaces) {
            ...
            const evalResult = surf && surf.a2p3d ? evalA2P3D(surf.a2p3d, lookup) : null;
            if (evalResult && surf) {
                const surfParams = {
                ...
                if (surf.type === 'PLANE') {
                    ...
                    if (bounds) {
                        const S = 0.001;
                        const scaleLoop = loop => loop.map(p => [p[0]*S, p[1]*S, p[2]*S]);
                        surfParams.outerBoundary = scaleLoop(bounds.outer);
                        surfParams.holeBoundaries = bounds.holes.map(scaleLoop);
                    }
                }
    ...
    if (opts.output) {
        ...
        if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
        outPath = uniqueFilename(baseName + '_extracted', ext, outDir);
    } else {
